...
class BtAssetMove(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='New User')
    ...
    def create(self, vals):
        ...
        if vals.get('from_loc_id', False) or vals.get('to_loc_id', False):
            if result.from_loc_id == result.to_loc_id and not result.employee_id:
    ...
    def action_move(self):
        for move in self:
            ...
            move.state = 'done'
            if move.employee_id:
                if move.asset_id.name:
                    move.asset_id.name = move.employee_id
                    move.asset_id.asset_name = move.employee_id.name
        return True
