    ...
    def print_report(self):
        ...
        if self.date_from and self.date_to:
            if not self.attachment_id:
                ...
                'Po Date','Time RML','Delivery Addr','Vhcl No','Trpt Name','Status','Executive','Exec Mail', 'Reverted Date', 'Invoice Date']
                row_index += 1
            ...
            #     # https://github.com/python-excel/xlwt/blob/master/xlwt/Style.py
                ...
                if self.status :
                    line_ids = self.env['logistic.trail.line'].sudo().search([('delivery_date','>=',self.date_from),('delivery_date','<=',self.date_to),('state','=',self.status)])
                else :
                    line_ids = self.env['logistic.trail.line'].sudo().search([('delivery_date','>=',self.date_from),('delivery_date','<=',self.date_to)])
                ...
                if line_ids:
                    ...
                    for line_id in line_ids:
                        ...
                        if line_id:
                            ...
                            worksheet.write(row_index, 14,line_id.state or '',  base_style )
