...
class sales_meet(models.Model):
    ...
    name = fields.Char('Meeting Subject', states={'done': [('readonly', True)]}, required=False, store=True, track_visibility='onchange') #, states={'done': [('readonly', True)]}, required=False
    ...
    # title_action = fields.Char('Next Activity Summary', track_visibility='onchange')
    ...
    def write(self, values, context=None):
        ...
        # print "koooooooooooooooooooooooooooooooooooooooo", len(values) , values
    ...
    #@api.onchange('checkin_lattitude')
    #def onchange_latlong(self):
        #
        #for event in self:
            #print "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK" , event.checkin_lattitude
            #if event.checkin_lattitude != 0.0:
                #print "ggggggggggggggggggggggggggggggggggggggggggggg" , event.checkin_lattitude
                #event.status = 'open'
                #event.next_flag = False
    ...
    def process_update_address_scheduler_queue(self):
        ...
        for rec in self.sudo().search([('status', '!=','draft'),('expense_date', '>=',daymonth),('reverse_location', '=',False)]) : #,('checkin_lattitude','=',True),('checkin_longitude','=',True)
