            ...
            if not self.attachment_id:
                ...
                if self.user_ids:
                    ...
                    user_ids = self.env['res.users'].sudo().search([
                        ('id','in',user_id),('active','=',True),('company_id','=',self.company_id.id)])

                else:


                    user_ids = self.env['res.users'].search([('active','=',True),('company_id','=',self.company_id.id)])
                ...
                for user_id in user_ids:
                    ...
                    for rec in intervals:
                        ...
                        if calendar_ids:
                            ...
                            elif len(calendar_ids) <= 4 and len(calendar_ids) > 0 :
                                attendance_status = 'H'
                                present_days = 0.5
                        ...
                        worksheet.write(row_index, column_index + int(rec.day) , attendance_status or '', base_style_green if attendance_status == 'P' \
                         else (base_style_yellow if attendance_status == 'H'  else (base_style_orange if attendance_status == 'L'  else base_style)) )
