...
class sales_meet(models.Model):
    ...
    def process_update_distance_scheduler_queue(self):
        ...
        print "JJJJJJJJJJJJ" , len(meeting_ids)
        for rec in meeting_ids: #self.search([('status', '!=','draft')]) :
            if rec.reverse_location and not rec.distance:
                # print "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" , rec.id , rec.user_id.name , rec.expense_date , rec.user_id
                users.append(rec.user_id.id) 
                location.append(rec.id) 

                # print "222222222222222222222222222222222222" , users , location
                for i, j in zip(users, location):
                    location_dict.setdefault(i, []).append(j)


                # print "333333333333333333333333333333333333" , location_dict

        # print "444444444444444444444444444" , location_dict

        for key , values in location_dict.iteritems():
            print "key: {}, value: {}".format(key, values)
            # print "5555555555555555555555" , len(location_dict) , location_dict[key]
            location_list = list(set(location_dict[key]))
            print "KKKKKKKKKKKKKKKKK" , location_list

            final_loc_list = [(location_list[i],location_list[i+1]) for i in range(0,len(location_list)-1)]
            print "mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm" , final_loc_list
            for records in final_loc_list:
                print "lllllllllllllllllllllllllllll" , records
                record_list = self.search([('id','in',records)])
                print "nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn" , record_list
                for res in record_list:
                    print "kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk" , res.reverse_location





            # for records in location_list:
            #     print "6666666666666666666666666666" , records

            # if len(key) > 0:
            #     location_dict[len(key)]
