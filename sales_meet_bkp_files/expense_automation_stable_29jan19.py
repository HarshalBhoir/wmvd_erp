...
class expense_automation(models.Model):
	...
	def action_expense_report(self):
		...
		if self.user_id:
			hr_expense = self.env['hr.expense.sheet'].sudo().search([('create_date', '>=', self.start_date), ('create_date', '<=', self.end_date), 
								('state', '=', self.expense_state), ('create_uid', '=', self.user_id.id)],order="create_uid, create_date asc")
		else:
			hr_expense = self.env['hr.expense.sheet'].sudo().search([('create_date', '>=', self.start_date), ('create_date', '<=', self.end_date), 
	...
	def expense_automation_webservice(self):
		...
		expense_invoice_filter = self.expense_automation_line_one2many.sudo().search([("expense_automation_id","=",self.id),("state","=",'approve')])
		...
		for rec in expense_invoice_filter:
			# print "lllllllllllllllllllllllll" , rec.expense_name.expense_line_ids[0].product_id.id
			# c_charge_id = self.env['wp.erp.charge'].sudo().search([("product_charge_id","=",rec.expense_name.expense_line_ids.product_id.id),
			# 													   ("company_id","=",self.company_id.id)]).c_charge_id
			# c_charge_id = rec.expense_name.expense_line_ids[0].product_id.erp_charge_one2many.search([("company_id","=",self.company_id.id)]).c_charge_id
		...
		for partner in partner_ids:
			...
			payment_body = """<_0:operations>
					...
					</_0:operation>"""  % ( self.ad_org_id.ad_org_id ,C_DocTypeTarget_ID, C_DocType_ID, daymonth, daymonth, partner, M_PriceList_ID,crm_description2)
			...
			for line_rec in vals:
				if partner == line_rec[0]:
					...
					# print "kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk" , partner , line_rec[0] , C_Charge_ID
...
# <_0:val>1001816</_0:val>
			...
			if log.find('IsRolledBack') is not -1:
				...
				raise ValidationError("Error Occured. Kindly Contact IT Helpdesk")
