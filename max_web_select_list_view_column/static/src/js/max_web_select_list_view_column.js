...
ListView.include({
	...
	if (session.is_superuser) {
		...
		model_res_users.call("has_group", ["max_web_select_list_view_column.group_columns_privilege"]).done(function(can_listcolumns) {
		    if (!can_listcolumns) {
		        self.$buttons.find('.oe_select_columns').hide();
		    }
		});
	}
