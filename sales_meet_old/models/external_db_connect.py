...
class external_db_connect(models.Model):
  ...
  #         ( SELECT record_id FROM fact_acct WHERE ad_table_id=318 GROUP BY ad_table_id, record_id, account_id, line_id, ad_org_id, ad_client_id\
    ...
    if self.config_id:
      ...
      try:
        ...
        if self.entry_type == 'invoice':
          ...
          # pg_cursor.execute("select docstatus from C_Invoice where   C_Invoice_ID = %s and documentno = %s  and docstatus !='RE' ",(idempiere_id,documentno))
          ...
          pg_cursor.execute("select C_AllocationLine_ID from adempiere.C_AllocationLine where C_Invoice_ID = %s ",[idempiere_id])
        ...
        #---------------------------------------------- # Payment Entry --------------------------------------------#
          ...
          pg_cursor.execute("select C_AllocationLine_ID  from adempiere.C_AllocationLine where   C_Payment_ID = %s",[idempiere_id])
          allocation = pg_cursor.fetchall()
          
          if len(allocation) > 0 and not self.partner_update:
            raise UserError("Allocation is present for this Entry. Please reset the allocation")

          pg_cursor.execute("select ad_client_id  from adempiere.C_Payment where   C_Payment_ID = %s",[idempiere_id])
          ad_client_id = pg_cursor.fetchall()

          print "oooooooooooooooooooooooooooooooooo" , ad_client_id

          self.ad_client_id = ((str(ad_client_id[0][0])).split('.'))[0] 

          pg_cursor.execute("select C_Payment_ID,dateacct, (select c_bpartner.name from adempiere.c_bpartner where c_bpartner.c_bpartner_id = cb.c_bpartner_id ), \
                              documentno, payamt,payamt, docstatus , posted  , \
                              processed from adempiere.C_Payment cb where C_Payment_ID=%s and documentno=%s",(idempiere_id,documentno))
        ...
        #---------------------------------------------- # Production Entry --------------------------------------------#
          ...
          pg_cursor.execute("select M_Production_ID,MovementDate, (select M_Product.name from adempiere.M_Product where M_Product.M_Product_ID = mp.M_Product_ID ), \
                              documentno, QtyBatchSize,productionqty, docstatus , posted  , \
                               processed from adempiere.M_Production mp where M_Production_ID=%s and documentno=%s",(idempiere_id,documentno))
        ...
        #---------------------------------------------- # Shipment Entry --------------------------------------------#
          ...
          pg_cursor.execute("select M_InOut_ID, dateordered ,  (select c_bpartner.name from adempiere.c_bpartner where c_bpartner.c_bpartner_id = mio.c_bpartner_id )\
                               ,documentno , totalpricelist,totalnetprice, docstatus , posted  , \
                               processed from  adempiere.M_InOut mio where  M_InOut_ID=%s and documentno=%s ",(idempiere_id,documentno))
        ...
        if self.entry_type and (self.employee_id or self.requester ):
            if self.entry_type == 'invoice':
              ...
              entry_type = 'Shipment'
            rep_name = entry_type + " Reactivation by "  + (self.employee_id.name if self.employee_id.name else self.requester)
      ...
      try:
        ...
        if self.entry_type == 'invoice':
          if self.movement_date == True:
            pg_cursor.execute("update adempiere.C_Invoice set dateacct = %s , dateinvoiced = %s where C_Invoice_ID=%s and documentno=%s",(changed_date, changed_date ,idempiere_id,documentno))
          else: 
            pg_cursor.execute("update adempiere.C_Invoice set docstatus = 'DR' , posted = 'N' , processed = 'N' where C_Invoice_ID=%s and documentno=%s",(idempiere_id,documentno))
        ...
        if self.entry_type == 'payment':
          ...
          if self.partner_update:
            pg_cursor.execute("select c_bpartner_id from adempiere.c_payment where   C_Payment_ID = %s and documentno=%s",(idempiere_id,documentno))
            ...
            pg_cursor.execute("update adempiere.C_Payment set c_bpartner_id = (select c_bpartner_id from adempiere.c_bpartner where value = %s and ad_client_id = ( select ad_client_id from adempiere.c_payment where c_payment_id = %s ))\
             where  documentno=%s and c_payment_id=%s",(value,idempiere_id,documentno,idempiere_id))
            self.c_bpartner_id = ''
          else:
            pg_cursor.execute("update adempiere.C_Payment set docstatus = 'DR' , posted = 'N' , processed = 'N' where C_Payment_ID=%s and documentno=%s",(idempiere_id,documentno))
        ...
        if self.entry_type == 'production':
          if self.movement_date == True:
            pg_cursor.execute("update adempiere.M_Production set MovementDate = DatePromised  where M_Production_ID=%s and documentno=%s",(idempiere_id,documentno))
          else:
            pg_cursor.execute("update adempiere.M_Production set docstatus = 'CO'  where M_Production_ID=%s and documentno=%s",(idempiere_id,documentno))
        ...
        if self.entry_type == 'shipment':
          ...
          if self.condition == 'status_change':
            pg_cursor.execute("update adempiere.M_InOut set docstatus = %s  where M_InOut_ID=%s and documentno=%s",(docstatus,idempiere_id,documentno))

          # if self.movement_date == True:
          #   pg_cursor.execute("update M_InOut set dateacct = %s , movementdate = %s where M_InOut_ID=%s and documentno=%s",(changed_date, changed_date ,idempiere_id,documentno))
          # else:
  ...
  def update_line_data_from_database(self):
    ...
    if self.config_id:
      try:
        ...
        if self.entry_type == 'invoice':
          pg_cursor.execute("update adempiere.C_InvoiceLine set processed = 'N' where C_Invoice_ID=%s",[idempiere_id])
    ...
    if self.config_id:
      try:
        ...
        if self.entry_type == 'payment' and self.ad_client_id:
          ...
          pg_cursor.execute("select docstatus from adempiere.c_payment where  documentno ilike '"+documentno+"%' and ad_client_id = '"+self.ad_client_id+"'")
          docstatus_check = pg_cursor.fetchall()
          ...
          pg_cursor.execute("update adempiere.C_Payment set c_bpartner_id = Null where documentno ilike '"+documentno+"%' and ad_client_id = '"+self.ad_client_id+"'")
...
class external_db_configuration(models.Model):
  ...
  def check_connection(self):
    ...
    if self.ip_address:
      ...
      if len(obj) > 0 :
          raise ValidationError(_("Default Database have already been set. Multiple Database Connections are not allowed"))

      try:

        conn_pg = psycopg2.connect(dbname= self.database_name, user=self.username, password=self.password, host= self.ip_address, port= self.port)
        pg_cursor = conn_pg.cursor()
...
class wp_grn_import(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env['res.company']._company_default_get('wp.grn.import'))
    ...
    def add_lines(self):
        ...
        if self.state == 'draft':
            ...
            del reader_info[0]
            values = {}
            # wp_grn_import.write({'imported': True,})
      ...
      if config_id:
        try:
          ...
          for res in self.grn_import_line_one2many:
            ...
            query_grn = main_query + ( inbound if res.inbound_no else '' ) + (  (', ' +  grn_no ) if res.grn_no else '' ) + \
                        ( (', ' + grn_date) if res.grn_date else '' ) + ( (', ' + ssc_date)   if res.ssc_date  else '' )  + ( ( ', ' + remarks ) if res.remarks else '' ) +  end_query


            print "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" , query_grn
