...
class asset_simple_report(models.TransientModel):
    _name = 'asset.simple.report'
    _description = "Asset Simple Report"


    name = fields.Char(string="AssetSimpleReport")
    ...
    employee_ids = fields.Many2many('hr.employee', 'assets_simple_report_hr_employee_rel', string='Employee')
    repname = fields.Char(string="AssetSimpleReport")
    ...
    def print_report(self):
        ...
        if not self.attachment_id:
            ...
            worksheet = workbook.add_sheet('Asset Simple')
            fp = StringIO()
            ...
            if self.date_from == self.date_to:
                rep_name = "Assets Simple Report(%s) - %s" % (date_from,second_heading)
            else:
                rep_name = "Assets Simple Report(%s|%s) - %s"  % (date_from, date_to,second_heading)
            self.name = rep_name
            ...
            'Physical Location','Department','Company','Purchase Date', 'Purchase Value', 'Tax Value', 'Total Value',
            'Manufacturer', 'Warranty Start', 'Warranty End']
            row_index += 1
            ...
            for asset_id in asset_ids:
                if asset_id:
                    ...
                    worksheet.write(row_index, 9,asset_id.purchase_date or '',  base_style_green )
                    worksheet.write(row_index, 10,asset_id.purchase_value or '',  base_style_green )
                    worksheet.write(row_index, 11,asset_id.tax_value or '',  base_style_green )
                    worksheet.write(row_index, 12,asset_id.total_value or '',  base_style_green )
                    worksheet.write(row_index, 13,asset_id.manufacturer or '',  base_style_orange )
                    worksheet.write(row_index, 14,asset_id.warranty_start or '',  base_style_orange )
                    worksheet.write(row_index, 15,asset_id.warranty_end or '',  base_style_orange )
